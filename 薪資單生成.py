...
try:#指定檔案位置
    ...
    path_xlsx = os.path.join(path,f'{soure_file}.xlsx')
    print(path_xlsx)
...
try:#載入檔案
    ...
    try:
        decryptedwb = loadencrypted_excel('薪資表.xlsx')
...
try:#取值
    ...
    for table_row in range(star_row, end_row):#指定ROW循環範圍
        ...
        for table_col in range (3, sheet.max_column+1):#指定COL循環範圍
            old_text = str(sheet.cell(row= star_row-1, column=table_col).value) #取得標題名當作範本WORD中的待替換文本
